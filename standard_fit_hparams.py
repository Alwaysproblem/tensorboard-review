...
# if you writing model with Keras this code below is optinal.
# def run(logdir,hparams):
#     with tf.summary.create_file_writer(logdir).as_default():
#         hp.hparams(hparams)   # record the values used in this trial
#         accuracy = train(logdir,hparams)
#         tf.summary.scalar(Metric_acc, accuracy, step=1) # w.r.t line 39. without this line the Acc has no values.

def run(logdir,hparams):
    return train(logdir,hparams)
