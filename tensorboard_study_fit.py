...
# with tf.summary.create_file_writer('./logs/text_hp/hparam_tuning').as_default():
#     hp.hparams_config(
...
def train(logdir, hparams):
    ...
    history = model.fit(x_train, y_train,
                        ...
                        verbose=1,
                        callbacks=[tf.keras.callbacks.TensorBoard(logdir, histogram_freq = 1, profile_batch=3),
                                   hp.KerasCallback(logdir, hparams)])
    _, acc = model.evaluate(test_x, text_y)
    return acc

def run(logdir, hpara):
    train(logdir, hpara)
...
for num_units in hp_num_units.domain.values:
    for optimizer in hp_optimizers.domain.values:
        ...
        run('logs/text_hp/hparam_tuning/' + run_name, hparams)
        session_num += 1
